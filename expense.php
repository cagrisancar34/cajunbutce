...
if (isset($_POST['action']) && $_POST['action'] == 'expense') {
    ...
    echo '<div class="notice notice-success">Gider kaydı eklendi.</div>';
}
echo '<h2>Gider Ekle</h2>';
echo '<form method="post">';
echo '<input type="hidden" name="action" value="expense">';
...
echo '</select> ';
echo '<button type="submit">Gider Ekle</button>';
echo '</form>';
$transactions = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}transactions WHERE type=%s ORDER BY date DESC LIMIT 10", 'expense'), ARRAY_A);
echo '<h3>Son Giderler</h3>';
